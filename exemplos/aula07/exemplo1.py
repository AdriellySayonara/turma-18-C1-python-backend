...
verificar_par_impar(-4) 
verificar_par_impar(-9) 

#somar com lista indefinida
def somar_lista(*args): #transforma em tupla
    return sum(args) 
...
def imprimir_info(**dic): #transforma em dicionario
    for chave, valor in dic.items(): #items() retorna chave e valor
        print(f"{chave}: {valor}")
imprimir_info(nome="João", idade=30, cidade="São Paulo")
imprimir_info(produto="Notebook", preco=2500.00, estoque=10)
...
#criar_usuario("Daniel", email="daniel_email.com") #erro, falta idade
